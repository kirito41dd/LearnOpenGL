...
int main(int argc, char **argv)
{
    ...
    // 编译着色器
    string vs,fs;
    vs = string(argv[1]) + "more3d.vs";
    fs = string(argv[1]) + "more3d.fs";
    ...
    glm::mat4 projection = glm::perspective(glm::radians(45.0f), (float)SCR_WIDTH/(float)SCR_HEIGHT, 0.1f, 100.0f); // 创建一个透视投影矩阵: 视野 宽高比 近平面 远平面距离
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // 创建 look at 矩阵
        view = glm::lookAt(glm::vec3(camX, 0.0f, camZ), // 摄像机位置
                           glm::vec3(0.0f, 0.0f, 0.0f), // 目标位置
                           glm::vec3(0.0f, 1.0f, 0.0f)); // 世界坐标的上方向
