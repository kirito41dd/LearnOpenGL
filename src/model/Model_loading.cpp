...
int main(int argc, char **argv)
{
    ...
    // ------------------------------------
    string shaderPath = root + "shader/model/";
    string vs,fs;
    vs = shaderPath + "model_loading.vs"; fs = shaderPath + "model_loading.fs";
    Shader ourShader(vs.c_str(), fs.c_str());
    // vs = shaderPath + "lamp.vs"; fs = shaderPath + "lamp.fs";
    // Shader lampShader(vs.c_str(), fs.c_str());

    // 模型
    string modelpath = root + "img/nanosuit/nanosuit.obj";
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShader.setFloat("light.cutOff", glm::cos(glm::radians(12.5f))); // 聚光角度大小，控制手电筒光柱
        ...
        ourShader.setMat4("model", worldModel);
